 ...
 * @author Victor Ojeda
 */
@WebServlet(name = "SelectSpriteModifier", urlPatterns = {"/SelectSpriteModifier"})
@MultipartConfig(fileSizeThreshold=1024*1024*2, // 2MB 
                 maxFileSize=1024*1024*10,      // 10MB
                 maxRequestSize=1024*1024*50,   // 50MB
                 location="/")
public class SelectSpriteModifier extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            ...
            label++;
            link_image.setLabel(Const.JLINKConst.IMAGE_LABEL + label);
            getServletContext().setAttribute("label", label);
            appPath = request.getServletContext().getRealPath("");
            saveDir = appPath + File.separator + Const.AppConst.MODIFIER_DIR;
    ...
    "                    </ul>");        
                out.println("<form id=\"selectSprite_button\" method=\"post\" action=\"AddLinkModifier\">\n" +
    "                    <input type=\"hidden\" name=\"sprite_x\" id=\"sprite_x\" value=\"\" />\n" +
    "                    <input type=\"hidden\" name=\"sprite_y\" id=\"sprite_y\" value=\"\" />\n" +
    "                    <input type=\"submit\" value=\"Do another action\"/>\n" +
    "                </form>\n" +
    "                </div>\n" +
    "                <div class=\"column\">\n" +
    "                <img class=\"previous_image\" id=\"img\" src=\"" + Const.AppConst.MODIFIER_DIR + File.separator + link_image.getPrevious_image() + ".jpeg\">\n" +
